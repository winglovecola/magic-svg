class svgCanvas {
    ...
    render() {
        return `<svg version="1.1" width="${this.width}" height="${this.height}" xmlns="http://www.w3.org/2000/svg" style="border:1px solid black">
${this.shapeElement}
${this.textElement}
</svg>`;
    }
...
class Rectangle extends Shape{
    constructor (fillColor = "green", x = 10, y = 10, width = 200, height = 150, stroke = "black", strokeWidth = 1, type = "Rectangle") {
        super (type, fillColor);
        this.x = x;
        this.y = y;
        this.width = width;
        this.height = height;
...
class Square extends Rectangle{
    constructor (fillColor = "green", x = 10, y = 10, width = 200, stroke = "black", strokeWidth = 1, type = "Square") {
        super (fillColor, x , y, width, width, stroke, strokeWidth, type);
    }

    render() {
        return `<rect x="${this.x}" y="${this.y}" width="${this.width}" height="${this.width}" stroke="${this.stroke}" stroke-width="${this.strokeWidth}" fill="${this.fillColor}" />`;
    }
}
...
export {Shape, Circle, Rectangle, Square, Triangle, textPrint, svgCanvas};
